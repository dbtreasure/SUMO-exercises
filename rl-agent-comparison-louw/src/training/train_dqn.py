#!/usr/bin/env python
"""Train a DQN agent on the SingleIntersectionTSCEnv.

Usage:
    uv run python -m src.training.train_dqn --config configs/single_int_dqn.yaml
    uv run python -m src.training.train_dqn --config configs/single_int_dqn.yaml --run-name my_run
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train DQN on TSC environment")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
...
def main() -> None:
    ...
    model = DQN(
        ...
        buffer_size=sb3_cfg["buffer_size"],
        batch_size=sb3_cfg["batch_size"],
        gamma=sb3_cfg["gamma"],
        exploration_initial_eps=sb3_cfg["exploration_initial_eps"],
        exploration_final_eps=sb3_cfg["exploration_final_eps"],
        target_update_interval=sb3_cfg["target_update_interval"],
        train_freq=sb3_cfg["train_freq"],
        gradient_steps=sb3_cfg["gradient_steps"],
